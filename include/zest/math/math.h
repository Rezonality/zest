...
template <class T>
struct hash<Zest::NVec2<T>>
{
    inline size_t operator()(const Zest::NVec2<T>& v) const
    {
        size_t seed = 0;
        hash<T> hasher;
        Zest::hash_combine(seed, hasher(v.x));
        Zest::hash_combine(seed, hasher(v.y));
        return seed;
    };
};

template <class T>
struct hash<Zest::NVec4<T>>
{
    inline size_t operator()(const Zest::NVec4<T>& v) const
    {
        size_t seed = 0;
        hash<T> hasher;
        Zest::hash_combine(seed, hasher(v.x));
        Zest::hash_combine(seed, hasher(v.y));
        Zest::hash_combine(seed, hasher(v.z));
        Zest::hash_combine(seed, hasher(v.w));
        return seed;
    };
};

template <class T>
struct hash<Zest::NRect<T>>
{
    inline size_t operator()(const Zest::NRect<T>& v) const
    {
        size_t seed = 0;
        hash<T> hasher;
        Zest::hash_combine(seed, hasher(v.topLeftPx));
        Zest::hash_combine(seed, hasher(v.bottomRightPx));
        return seed;
    };
};
