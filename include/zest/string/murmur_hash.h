...
constexpr uint32_t murmur_hash(const void * key, int len, uint32_t seed)
{
...
constexpr unsigned int invert_shift_xor(unsigned int hs, unsigned int s)
{
    assert(s >= 8 && s <= 16);
    unsigned hs0 = hs >> 24;
...
constexpr unsigned int murmur_hash_inverse(unsigned int h, unsigned int seed)
{
    const unsigned int m = 0x5bd1e995;
...
constexpr uint64_t murmur_hash_64(const void * key, uint32_t len, uint64_t seed)
{
